...
<body>
  ...
  //To fetch tags links
  $tagsQuery = "SELECT tags FROM tuby2";
  ...
  if (isset($_GET['hello'])) {
    $tagMeow = $_GET['meow'];
    $sql = "SELECT * FROM tuby2 WHERE tags LIKE '%$tagMeow%'";
  } else {
  ...
  echo '<div id="navbar">';
  echo '<a href="/youtube-tuby-v2/index.php">Home</a>';
  ...
  for ($i = 0; $i < count($tagsSingle); $i++) {
    if ($tagsUnique[$i]) { //To filter the empty array values
      if($tagsUnique[$i] == $_GET['meow']){ //For Default value in Select
        echo '<option value="/youtube-tuby-v2/index.php?hello=true&meow=' . $tagsUnique[$i] . '" selected>' . $tagsUnique[$i] . '</option>';
      }
      else{
        echo '<option value="/youtube-tuby-v2/index.php?hello=true&meow=' . $tagsUnique[$i] . '">' . $tagsUnique[$i] . '</option>';
      }
    }
  }

  echo '</select>
  <a href="/youtube-tuby-v2/insert/form.html" style="float:right;"> Insert </a>
  </div>';
